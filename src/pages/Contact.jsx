...
const Contact = () => {
    ...
    const handleSubmit = (e) => {
        ...
        emailjs.send(
            ...
            showAlert("Message sent failed", "danger");
        })
    };

    return (
        <section className="relative flex lg:flex-row flex-col max-container gap-14 items-stretch">
            ...
            <div className="flex-1 min-w[50%] flex flex-col">
                ...
                <form className="w-full flex flex-col gap-7 mt-14" onSubmit={handleSubmit}>
                    ...
                    <button type="submit" className="btn" disabled={isLoding} onFocus={handleFocus} onBlur={handleBlur}>{isLoding ? "Sending..." : "Send Message"}</button>
                </form>
            </div>

            <div className="lg:w-1/2 w-full md:h-[550px] h-[350px] flex justify-center items-center">
                <Canvas camera={{ position: [0, 0, 5], fov: 25, near: 0.1, far: 1000 }}>
                    <directionalLight intensity={2.5} position={[0, 0, 1]} />
                    <ambientLight intensity={0.5} />
                    <Suspense fallback={<Loader />}>
                        <Fox currentAnimation={currentAnimation} position={[0, 0, 0]} rotation={[12.6, -0.6, 0]} scale={[0.2, 0.2, 0.2]} />
                    </Suspense>
                </Canvas>
            </div>
        </section>
